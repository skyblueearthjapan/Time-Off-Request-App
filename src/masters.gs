 ...
 * workerIdが指定されている場合、既に使用済みの振替元出勤日は除外する。
 * @param {string} workerId 作業員ID（省略可。指定時は使用済み除外）
...
function api_getHolidayDays(workerId) {
  ...
  while (cursor <= endDate) {
    var key2 = fmtDate_(cursor);
    var dow = cursor.getDay();

    if (calCoveredDates[key2]) {
      var kubun = calMap[key2];
      if (kubun === '休日' || kubun === '祝日') {
        result.push({ date: key2, label: kubun });
      } else if (kubun === '出勤土曜') {
        // 出勤土曜は勤務日 → 候補に含めない
      } else if (dow === 0 || dow === 6) {
        result.push({ date: key2, label: dow === 0 ? '日曜' : '土曜' });
      }
    } else {
      if (publicHolidays[key2]) {
        result.push({ date: key2, label: '祝日（' + publicHolidays[key2] + '）' });
      } else if (dow === 0) {
        result.push({ date: key2, label: '日曜' });
      } else if (dow === 6) {
        result.push({ date: key2, label: '土曜' });
      }
    }
    cursor.setDate(cursor.getDate() + 1);
  }

  // 既に使用済みの振替元出勤日を除外（workerId指定時のみ・取消済は再利用可）
  if (workerId) {
    try {
      ...
      if (lrSh && lrSh.getLastRow() >= 2) {
        ...
        if (subCol !== undefined && stCol !== undefined && ltCol !== undefined && wIdCol !== undefined) {
          ...
          result = result.filter(function(c) { return !usedDates[c.date]; });
        }
      }
    } catch (e) {
